...
async function generate(manifest: string, output: string) {
  ...
  fs.writeFileSync(`${output}/types.json`, converted.typesContent, { encoding: 'utf8' });

  console.log(`Writing index to ${output}/index.json`);
  fs.writeFileSync(`${output}/index.json`, converted.indexContent, { encoding: 'utf8' });

  console.log(`Writing documentation to ${output}/index.md`);
  fs.writeFileSync(`${output}/index.md`, converted.documentationContent, { encoding: 'utf8' });
}
...
export async function run() {
    ...
    .command('generate <manifest> <output> [options]', 'Generate Bicep extension from Radius Resource Provider manifest.', yargs => {
      return yargs
          .positional('manifest', {
            demandOption: true,
            type: 'string'
