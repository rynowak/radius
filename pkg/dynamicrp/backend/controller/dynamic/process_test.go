...
func createController(t *testing.T) (*Controller, *store.MockStorageClient, *engine.MockEngine, *processors.MockResourceClient, *configloader.MockConfigurationLoader, *MockapiVersionsClient) {
	...
	// UCP types
	apiVersionsClient := NewMockapiVersionsClient(ctrl)
	...
	}, storageClient, engine, client, configLoader, apiVersionsClient
}
...
func mockResourceProvider(apiVersionsClient *MockapiVersionsClient) {
	response := v20231001preview.APIVersionsClientGetResponse{
		APIVersionResource: v20231001preview.APIVersionResource{
			Name:       to.Ptr(apiVersion),
			Properties: &v20231001preview.APIVersionProperties{},
		},
	}

	apiVersionsClient.EXPECT().
		Get(gomock.Any(), "local", "Applications.Test", "testResources", apiVersion, gomock.Any()).
		Return(response, nil).
		Times(1)
}
...
func Test_Run_InvalidOperation(t *testing.T) {
	c, _, _, _, _, apiVersionsClient := createController(t)

	mockResourceProvider(apiVersionsClient)
...
func Test_Run_Put(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		c, storageClient, e, _, configLoader, apiVersionsClient := createController(t)
