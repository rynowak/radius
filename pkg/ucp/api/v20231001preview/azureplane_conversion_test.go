...
func Test_AzurePlane_ConvertVersionedToDataModel(t *testing.T) {
...
func Test_AzurePlane_ConvertDataModelToVersioned(t *testing.T) {
	conversionTests := []struct {
		filename string
	...
	for _, tt := range conversionTests {
		t.Run(tt.filename, func(t *testing.T) {
			rawPayload := testutil.ReadFixture(tt.filename)
			dm := &datamodel.AzurePlane{}
			err := json.Unmarshal(rawPayload, dm)
			require.NoError(t, err)

			resource := &AzurePlaneResource{}
