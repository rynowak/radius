...
func RegisterHandler(ctx context.Context, opts HandlerOptions, ctrlOpts ctrl.Options) error {
	...
	if duplicated {
		logger.Info(fmt.Sprintf("Warning: skipping handler registration because '%s %s' has been registered before.", opts.Method, opts.Path), "operation", opts.OperationType)
		return nil
	}
