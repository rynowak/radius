...
const (
	...
	containerImage        = "someimage:latest"
...
// makeDefaultResource returns a default container resource.
func makeDefaultResource() *datamodel.ContainerResource {
	...
	return makeResource(properties)
}

// makeDefaultDeployment returns a default deployment. This matches the defaults
// used by makeDefaultResource.
//
// This should be used in all tests, and modifed to match the desired test scenario.
func makeDefaultDeployment(opts *Options) *appsv1.Deployment {
	deployment := baseDeployment(kubeutil.ObjectManifest{}, applicationName, resourceName, "Applications.Core/containers", opts)
...
//
// If you add minor features, or changed default behaviors, update this test.
func Test_RenderKubernetes_Defaults(t *testing.T) {
	options := makeOptions()

	resource := makeDefaultResource()
	resource.Properties.Container.Env = map[string]string{
		"b": "value2",
		"a": "value1",
	}

	output, err := RenderKubernetes(testcontext.New(t), resource, options)
	require.NoError(t, err)

	expectedDeployment := makeDefaultDeployment(options)
	expectedDeployment.Spec.Template.Spec.Containers[0].Env = []corev1.EnvVar{
		{
			Name:  "a",
			Value: "value1",
		},
	...
	verifyDeployment(t, output, expectedDeployment)

	// Every
	t.Run("verify identity", func(t *testing.T) {
		identity := output.Identity.(*kubernetesServiceAccount)
		require.NotNil(t, identity)
	})

	t.Run("verify others", func(t *testing.T) {
		require.Nil(t, output.Network)
		require.Nil(t, output.Secrets)
		require.Nil(t, output.Storage)
	})
}
...
// 	ctx := testcontext.New(t)
// 	output, err := RenderKubernetes(ctx, resource, Options{Dependencies: dependencies})
// 	require.NoError(t, err)
...
// 		expected.CreateResource.Dependencies = []string{rpv1.LocalIDServiceAccount, rpv1.LocalIDKubernetesRole, rpv1.LocalIDKubernetesRoleBinding}
...
// 	ctx := testcontext.New(t)
// 	output, err := RenderKubernetes(ctx, resource, Options{Dependencies: dependencies, Environment: renderers.EnvironmentOptions{Namespace: "default"}})
// 	require.NoError(t, err)
...
// 	}
// 	ctx := testcontext.New(t)
// 	output, err := RenderKubernetes(ctx, resource, Options{Dependencies: dependencies, Environment: testEnvironmentOptions})
// 	require.NoError(t, err)
...
// 	}
// 	ctx := testcontext.New(t)
// 	_, err := RenderKubernetes(ctx, resource, Options{Dependencies: dependencies})
// 	require.NoError(t, err)
// }
...
// 	ctx := testcontext.New(t)
// 	renderOutput, err := RenderKubernetes(ctx, resource, Options{Dependencies: dependencies})
...
// 	ctx := testcontext.New(t)
// 	renderOutput, err := RenderKubernetes(ctx, resource, Options{Dependencies: dependencies, Environment: testEnvironmentOptions})
...
// 		resource := makeResource(t, properties)
// 		ctx := testcontext.New(t)
// 		output, err := RenderKubernetes(ctx, resource, renderOptionsEnvAndAppKubeMetadata())
...
// func renderOptionsEnvAndAppKubeMetadata() Options {
// 	dependencies := map[string]renderers.RendererDependency{}
...
func verifyDeployment(t *testing.T, output *compute.Deployment, expected *appsv1.Deployment) {
	t.Run("verify core (Kubernetes Deployment)", func(t *testing.T) {
		require.IsType(t, &kubernetesDeployment{}, output.Compute)
		deployment := output.Compute.(*kubernetesDeployment).Deployment
		require.NotNil(t, deployment)
		require.Equal(t, deployment, expected)
	})
}
