...
var (
	namespaceRegex    = regexp.MustCompile(`^[A-Z][A-Za-z0-9]+.[A-Z][A-Za-z0-9]+$`)
	...
	namespaceMessage    = "{0} must be a valid resource provider namespace. A resource provider namespace must contain two PascalCased segments separated by a '.'. Example: MyCompany.Resources"
...
func validateNamespace(fl validator.FieldLevel) bool {
	str := fl.Field().String()
	return namespaceRegex.Match([]byte(str))
}
