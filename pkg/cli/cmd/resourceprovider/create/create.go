...
rad resourceprovider create --from-file /path/to/input.yaml
`,
...
func (r *Runner) Run(ctx context.Context) error {
	...
	r.Output.LogInfo("Creating resource provider %s", r.ResourceProvider.Namespace)
	_, err = client.CreateOrUpdateResourceProvider(ctx, "local", r.ResourceProvider.Namespace, &v20231001preview.ResourceProviderResource{
	...
	for resourceTypeName, resourceType := range r.ResourceProvider.Types {
		r.Output.LogInfo("Creating resource type %s/%s", r.ResourceProvider.Namespace, resourceTypeName)
		_, err := client.CreateOrUpdateResourceType(ctx, "local", r.ResourceProvider.Namespace, resourceTypeName, &v20231001preview.ResourceTypeResource{
		...
		for apiVersionName := range resourceType.APIVersions {
			r.Output.LogInfo("Creating API Version %s/%s@%s", r.ResourceProvider.Namespace, resourceTypeName, apiVersionName)
			_, err := client.CreateOrUpdateAPIVersion(ctx, "local", r.ResourceProvider.Namespace, resourceTypeName, apiVersionName, &v20231001preview.APIVersionResource{
	...
	r.Output.LogInfo("Creating location %s/%s", r.ResourceProvider.Namespace, v1.LocationGlobal)
	_, err = client.CreateOrUpdateLocation(ctx, "local", r.ResourceProvider.Namespace, v1.LocationGlobal, &locationResource)
	if err != nil {
		return err
	}

	response, err := client.GetResourceProvider(ctx, "local", r.ResourceProvider.Namespace)
	if err != nil {
		return err
	}
